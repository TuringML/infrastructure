...
do_site2site_configure() {
  ...
  sed -i -e '$a\' ${NIFI_HOME}/conf/nifi.properties
  echo nifi.remote.input.port=${NIFI_REMOTE_INPUT_PORT:-8081} >> ${NIFI_HOME}/conf/nifi.properties
  ...
  echo 'nifi-0.nifi.nifi.svc.cluster.local nifi-0' >> /tmphosts
  echo 'nifi-1.nifi.nifi.svc.cluster.local nifi-1' >> /tmphosts
  echo 'nifi-2.nifi.nifi.svc.cluster.local nifi-2' >> /tmphosts
